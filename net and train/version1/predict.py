...
#y is actual label and z is prediction result(not changed to label)
...
#training process
def training_process(training_samples, training_labels, testing_samples, testing_labels, max_iterations, batch_size, learning_rate):
    
    #set the parameters, batch size, max_iterations
    ...
    return np.mean(np.array(training_accuracy[-5:])), np.mean(np.array(testing_accuracy[-5:]))
...
def training(net, features, labels, max_iterations, batch_size, learning_rate):
    ...
    #reorder the samples and labels
    np.random.seed(6)
    ...
    #train the net
    train_acc, test_acc =  training_process(training_samples, training_labels, testing_samples, testing_labels, max_iterations, batch_size, learning_rate)
